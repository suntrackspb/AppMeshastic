...
def main() -> None:
    ...
    def _on_closing():
        ...
        async def _shutdown():
            ...
            try:
                await asyncio.wait_for(node_manager.disconnect_all_mirrors(), timeout=3)
            except Exception:
                pass
...
# TODO 
# 1. Добавить систему уведомлений глобальную. И через нее из любого места приложения выбрасывать уведомления, например в угол окна. Можно уведомлять о трейсроутах, когда он завершается в фоне и других событиях, сообщениях на не активной ноде и т.п.
# 2. Пропадают лейблы и вкладки при переключение между нодами. Когда подключено 2 ноды, и тебе не не активную приходят сообщение, в списке подключенных появляется лейбл, но как только ты переключаешь на неё, лейблов на вкладках по каналам нету. Так же, сли были открыты вкладки с личкой, при переключение на другую ноду и обратно они тоже загрываются. В общем надо видимо хранить какое то состояние окна.
# 3. Есть функционал который при подключение ноды на  API отправляет запрос, для мониторинга использования приложения, Добавить версию приложения при отправке репорта о запуске приложения на сервер.
# 4. Добавить кнопку такую же по функционалу как появляется с конвертиком при наведение на ноду в списке в сайдбаре, при нажатие открывается вкладка с личной перепиской, такая же кнопка нужна в модалке инфы о ноде, так как она открывается ещё и при клике на ник в чате, это будет удобнее чем искать ноду в списке.
# 5. Так же надо сделать систему отслеживания в чате своей ноды, что имеется в виду, Если вдруг идет ответ, упоминание, реакция, в общем в пакете присутствует ID твоей ноды, то воспроизводить звуковое уведомление примерно как при входящих личных сообщениях, а так же можно использовать систему из 1 пункта.